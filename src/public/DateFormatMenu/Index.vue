<template>
    <el-form v-if="currentComponent" :mode="form" class="receiver-menu-warp" label-position="top" @click.native.stop>
  ...
  <script>
    ...
    export default {
      ...
      computed: {
        ...mapGetters(['activeComponent', 'storeList','dateStr','year']),
        currentComponent() {
      ...
      methods: {
        ...
        change(value){//选择之后改变text 内容，
           const {dateStr,year} =this.currentComponent //获取年/月/日
           this.currentComponent.props.text=value=='ssqjm'?dateStr:year
        },
        deletes(){
