...
const components = {
    ...
    mutations: {
        ...
        UPDATE_STORE_LIST(state, template) {
            state.storeLoading = true
            state.storeList = template.map((item) => {
        ...
        REMOVE_ACTIVE_COMPONENT(state) {
            const id = state.activeComponent.id
            if (id) {
                remove(state.storeList, 'id', id)
            } else {
                state.selected.instance.map((item) => {
